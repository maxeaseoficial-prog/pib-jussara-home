...
import { MonthlyProgramCalendar } from "./MonthlyProgramCalendar";
import { Reveal } from "./Reveal";

export function UpcomingEvents() {
  const services = useQuery({
    queryKey: programmingQueryKeys.publicServices,
    queryFn: ({ signal }) => loadPublicChurchServices(signal),
  });

  const cards = services.data
    ? services.data.map((service) => {
        const nextDate = nextServiceOccurrence(service);
        const badge = nextDate
          ? formatOccurrenceBadge(nextDate)
          : { day: "—", month: "—", weekday: "Programação" };
        return {
          id: service.id,
          title: service.title,
          category: service.category,
          description: service.description,
          time: formatServiceTime(service.service_time),
    ...
    : fallbackEvents.map((event, index) => ({ ...event, id: `fallback-${index}` }));
  ...
  return (
    <section id="programacao" className="bg-background py-20 sm:py-28 lg:py-32">
      <div className="mx-auto max-w-[1360px] px-5 sm:px-8 lg:px-12">
        ...
        {services.isError ? (
          <p className="mt-7 rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-900">
            A programação ao vivo não pôde ser carregada agora. Exibindo a programação de referência.
          </p>
        ) : null}

        {cards.length === 0 ? (
          <div className="mt-14 rounded-3xl border border-border bg-surface-soft px-6 py-10 text-center">
            <p className="font-extrabold text-green-950">A programação fixa está sendo atualizada.</p>
          </div>
        ) : (
                ...
                <article className="group flex h-full flex-col overflow-hidden rounded-3xl border border-border bg-surface transition-all duration-300 hover:-translate-y-1 hover:border-brand-green/50">
                  <div className="relative overflow-hidden">
                    ...
                    <span className="absolute left-4 top-4 flex flex-col items-center rounded-2xl bg-surface px-3 py-2 leading-none">
                      <span className="text-xl font-extrabold text-text-primary">{event.date}</span>
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    <p className="eyebrow text-green-700">{event.category}</p>
        ...
        <MonthlyProgramCalendar services={services.data ?? []} />
      </div>
    </section>
  );
}
